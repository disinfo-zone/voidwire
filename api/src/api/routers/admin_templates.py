...
from voidwire.models import AdminUser, AuditLog, PromptTemplate
...
def _template_dict(t: PromptTemplate) -> dict:
    return {
        ...
        "tone_parameters": t.tone_parameters,
        "notes": t.notes,
        "author": t.author,
...
async def list_templates(
    ...
    result = await db.execute(
        ...
        .where(PromptTemplate.is_active)
...
async def get_template(
    template_id: UUID, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)
):
...
async def get_template_versions(
    template_name: str, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)
):
    result = await db.execute(
        select(PromptTemplate)
...
async def create_template(
    ...
    result = await db.execute(
        select(func.coalesce(func.max(PromptTemplate.version), 0)).where(
            PromptTemplate.template_name == req.template_name
        )
    )
    ...
    t = PromptTemplate(
        ...
        tone_parameters=req.tone_parameters,
        notes=req.notes,
        author=user.email,
    )
    db.add(t)
