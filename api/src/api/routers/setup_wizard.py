...
from voidwire.models import AdminUser, LLMConfig, SetupState, SiteSetting
from voidwire.services.encryption import encrypt_value

from api.dependencies import get_db
from api.middleware.auth import generate_totp_secret, get_totp_uri, hash_password
...
async def create_admin(req: AdminCreateRequest, db: AsyncSession = Depends(get_db)):
    ...
    return {
        "status": "ok",
        "totp_uri": get_totp_uri(totp_secret, req.email),
...
async def configure_site(req: SiteConfigRequest, db: AsyncSession = Depends(get_db)):
    await _require_setup_incomplete(db)
    for key, val, cat in [
        ("site.title", {"value": req.site_title}, "general"),
        ("site.timezone", {"value": req.timezone}, "general"),
        ("pipeline.auto_publish", {"enabled": req.auto_publish}, "pipeline"),
    ]:
