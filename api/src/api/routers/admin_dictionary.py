...
def _meaning_dict(m: ArchetypalMeaning) -> dict:
    return {
        "id": str(m.id), "body1": m.body1, "body2": m.body2,
        "aspect_type": m.aspect_type, "event_type": m.event_type,
        "core_meaning": m.core_meaning, "keywords": m.keywords,
        "domain_affinities": m.domain_affinities, "source": m.source,
    }
...
async def get_meaning(meaning_id: UUID, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)):
...
async def create_meaning(req: MeaningRequest, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)):
    m = ArchetypalMeaning(
        body1=req.body1, body2=req.body2, aspect_type=req.aspect_type,
        event_type=req.event_type, core_meaning=req.core_meaning,
        keywords=req.keywords, domain_affinities=req.domain_affinities,
        source=req.source,
    )
    db.add(m)
    await db.flush()
...
async def update_meaning(meaning_id: UUID, req: MeaningUpdateRequest, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)):
    m = await db.get(ArchetypalMeaning, meaning_id)
    if not m:
        raise HTTPException(status_code=404, detail="Meaning not found")
    for field in ("body1", "body2", "aspect_type", "event_type", "core_meaning", "keywords", "domain_affinities", "source"):
...
async def delete_meaning(meaning_id: UUID, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)):
    m = await db.get(ArchetypalMeaning, meaning_id)
    if not m:
        raise HTTPException(status_code=404, detail="Meaning not found")
    await db.delete(m)
...
async def bulk_import(entries: list[MeaningRequest], db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)):
    created = []
    for req in entries:
        m = ArchetypalMeaning(
            body1=req.body1, body2=req.body2, aspect_type=req.aspect_type,
