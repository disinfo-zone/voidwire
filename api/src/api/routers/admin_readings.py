    ...
    return [
        {
            ...
            "date_context": r.date_context.isoformat(),
            "status": r.status,
            "title": (r.generated_standard or {}).get("title", ""),
            "published_at": r.published_at.isoformat() if r.published_at else None,
        }
...
async def get_reading(
    reading_id: UUID, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)
):
...
async def update_reading(
    ...
    if req.status:
        ...
        if req.status == "published":
            reading.published_at = datetime.now(UTC)
    ...
    db.add(
        AuditLog(
            user_id=user.id,
            action=f"reading.{req.status or 'edit'}",
            target_type="reading",
...
async def update_reading_content(
    reading_id: UUID,
    req: ReadingContentUpdateRequest,
    db: AsyncSession = Depends(get_db),
    user: AdminUser = Depends(require_admin),
):
    reading = await db.get(Reading, reading_id)
    ...
    if reading.published_standard and reading.generated_standard:
        if reading.published_standard != reading.generated_standard:
            diff["standard"] = {
                "generated_title": reading.generated_standard.get("title"),
                "published_title": reading.published_standard.get("title"),
                "body_changed": reading.published_standard.get("body")
                != reading.generated_standard.get("body"),
            }
...
async def regenerate_reading(
    ...
    mode_map = {
        "prose_only": RegenerationMode.PROSE_ONLY,
        "reselect": RegenerationMode.RESELECT,
        "full_rerun": RegenerationMode.FULL_RERUN,
    }
