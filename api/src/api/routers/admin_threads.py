...
from voidwire.models import CulturalThread, ThreadSignal, CulturalSignal, AdminUser
...
def _thread_dict(t: CulturalThread) -> dict:
    return {
        "id": str(t.id), "canonical_summary": t.canonical_summary,
        "domain": t.domain, "active": t.active,
...
async def get_thread(thread_id: UUID, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)):
...
async def update_thread(thread_id: UUID, req: ThreadUpdateRequest, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)):
    t = await db.get(CulturalThread, thread_id)
    if not t:
        raise HTTPException(status_code=404, detail="Thread not found")
...
async def deactivate_thread(thread_id: UUID, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)):
    t = await db.get(CulturalThread, thread_id)
    if not t:
        raise HTTPException(status_code=404, detail="Thread not found")
    t.active = False
...
async def get_thread_signals(thread_id: UUID, db: AsyncSession = Depends(get_db), user: AdminUser = Depends(require_admin)):
    t = await db.get(CulturalThread, thread_id)
    if not t:
        raise HTTPException(status_code=404, detail="Thread not found")
    ...
    return [
        {
            ...
            "date_seen": row.ThreadSignal.date_seen.isoformat() if row.ThreadSignal.date_seen else None,
