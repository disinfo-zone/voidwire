...
def _recency_weighted_centroid(old: list[float], new: list[float], decay: float = CENTROID_DECAY) -> list[float]:
...
async def run_thread_stage(signals: list[dict], date_context: date, session: AsyncSession) -> list[dict]:
    ...
    reactivation_cutoff = date_context - timedelta(days=DEACTIVATION_DAYS * 3)
        ...
        if best_match:
            ...
            if best_match.centroid_embedding is not None:
                ...
                if old_centroid and len(old_centroid) == len(emb):
                    best_match.centroid_embedding = _recency_weighted_centroid(old_centroid, emb)
    ...
    cutoff = date_context - timedelta(days=DEACTIVATION_DAYS)
