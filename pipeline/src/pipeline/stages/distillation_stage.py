...
async def _get_llm_client(session: AsyncSession, slot: str, *, timeout: float = 120.0) -> LLMClient:
    result = await session.execute(
        select(LLMConfig).where(LLMConfig.slot == slot, LLMConfig.is_active)
    )
...
async def run_distillation_stage(
    ...
    try:
        ...
        signals = await generate_with_validation(
            client, "distillation", [{"role": "user", "content": prompt}], _validate_signals
        )
        await client.close()
        ...
        signals = [
            {
                "summary": a.get("title", ""),
                "domain": a.get("domain", "anomalous"),
                "intensity": "minor",
                "directionality": "stable",
                "entities": [],
                "source_refs": [],
            }
    ...
    for i, signal in enumerate(signals):
        normalized = _normalize_signal(
            signal if isinstance(signal, dict) else {"summary": str(signal)}
        )
        sid = f"sig_{date_context.strftime('%Y%m%d')}_{run_id.hex[:8]}_{i + 1:03d}"
        db_signal = CulturalSignal(
            id=sid,
